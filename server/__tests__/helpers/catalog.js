...
async function insertCategory({ name, default_storage = 'fridge',
    ...
    const res = await pool.query(
        `INSERT INTO categories (name, default_storage, pantry_days, fridge_days, freezer_days, household_id)
         VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
        [name, default_storage, pantry_days, fridge_days, freezer_days, household_id]
    );
...
async function insertFoodType({ name, category_id = null, default_storage = null,
                                ...
                                household_id = null }) {
    const res = await pool.query(
        `INSERT INTO food_types (name, category_id, default_storage, pantry_days, fridge_days, freezer_days, household_id)
...
async function insertBrandProduct({ brand, food_type_id, default_storage = null,
                                    ...
                                    household_id = null }) {
    const res = await pool.query(
        `INSERT INTO brand_products (brand, food_type_id, default_storage, pantry_days, fridge_days, freezer_days, household_id)
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`,
        [brand, food_type_id, default_storage, pantry_days, fridge_days, freezer_days, household_id]
    );
    return res.rows[0].id;
}
